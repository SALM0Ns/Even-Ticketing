...
async function fixSeatSelectionData() {
  try {
    console.log('🔧 Fixing seat selection data...\n');
    ...
    for (const movie of movies) {
      ...
      // Use findByIdAndUpdate to ensure the update works
      ...
      console.log(`  ✅ ${movie.name}: ${updatedMovie.showDates.length} shows, ${updatedMovie.seating.takenSeats.length}/${updatedMovie.seating.totalSeats} seats taken`);
    }
    ...
    for (const play of stagePlays) {
      ...
      const updatedPlay = await StagePlays.findByIdAndUpdate(
        play._id,
        { 
          showDates: showDates,
          seating: {
      ...
      console.log(`  ✅ ${play.name}: ${updatedPlay.showDates.length} shows, ${updatedPlay.seating.takenSeats.length}/${updatedPlay.seating.totalSeats} seats taken`);
    }
    ...
    for (const concert of orchestra) {
      ...
      const updatedConcert = await LiveOrchestra.findByIdAndUpdate(
        concert._id,
        { 
          showDates: showDates,
          seating: {
      ...
      console.log(`  ✅ ${concert.name}: ${updatedConcert.showDates.length} shows, ${updatedConcert.seating.takenSeats.length}/${updatedConcert.seating.totalSeats} seats taken`);
    }

    console.log('\n🎉 Seat selection data fixed successfully!');
    ...
    console.error('❌ Error fixing seat selection data:', error);
  } finally {
    mongoose.connection.close();
  }
}

fixSeatSelectionData();
